...
export const Item: React.FC<ItemProps> = ({ position = [0, 0, 0], scale = 1 }) => {
  ...
  const currentFireworkRef = useRef<FireworkSelection | null>(null)
  ...
  if (currentFireworkRef.current === null) {
    currentFireworkRef.current = createFireworkSelection()
  }

  if (particleSeedsRef.current.length === 0) {
    particleSeedsRef.current = Array.from({ length: PARTICLE_COUNT }, (_, index) => createParticleSeed(index, currentFireworkRef.current!))
  }
  ...
  const transitionTo = (nextPhase: FireworkPhase, startedAt = performance.now()) => {
  ...
  const prepareNextFirework = () => {
    const nextFirework = createFireworkSelection()
    currentFireworkRef.current = nextFirework
    particleSeedsRef.current = Array.from({ length: PARTICLE_COUNT }, (_, index) => createParticleSeed(index, nextFirework))
  }
  ...
  return (
    <group ref={groupRef} position={position} scale={scale}>
      ...
      <pointLight ref={burstLightRef} position={[0, burstHeight, 0]} color="#ffd166" distance={14} intensity={0} />
